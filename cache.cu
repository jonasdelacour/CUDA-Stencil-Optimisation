...
public:
    ...
    std::vector<std::pair<void**,size_t>> d_buffers = {{(void**)&d_P, sizeof(real_t)},{(void**)&d_Q, sizeof(real_t)}, {(void**)&d_PQ,sizeof(real2_t)}, {(void**)&d_PQdt, sizeof(real2_t)}}; 
    ...
    void copyIn(){
        cudaMemcpy(d_PQ, PQ, sizeof(real2_t)*NX*NY, cudaMemcpyHostToDevice);
    }
...
void derivative(Chemicals& c, real2_t* s_PQ){
    real2_t* PQ = c.d_PQ;
    auto ylim = c.NY + (blockDim.y - c.NY % blockDim.y) + 1;
    auto xlim = c.NX + (blockDim.x - c.NX % blockDim.x) + 1;
    for (auto gtidy = threadIdx.y + blockIdx.y * blockDim.y + 1; gtidy < ylim; gtidy += gridDim.y*blockDim.y)
    for (auto gtidx = threadIdx.x + blockIdx.x * blockDim.x + 1; gtidx < xlim; gtidx += gridDim.x*blockDim.x)
    { 
        auto tid = gtidx + gtidy*c.NX;
        bool valid_range = gtidx < c.NX - 1 && gtidy < c.NY -1;
        ...
        if (valid_range)
        {
            if (threadIdx.x == 0)               s_PQ[left]  = PQ[tid - 1];
            if (threadIdx.x == blockDim.x -1)   s_PQ[right] = PQ[tid + 1];
            if (threadIdx.y == blockDim.y -1)   s_PQ[down]  = PQ[tid + c.NX];
            if (threadIdx.y == 0)               s_PQ[up]    = PQ[tid - c.NX];
        }
        cg::sync(cg::this_thread_block());


        if (valid_range){    
            real_t dPdt = c.D_p * ( s_PQ[left].x + s_PQ[right].x + s_PQ[up].x + s_PQ[down].x - real_t(4.0)*s_PQ[center].x) / (c.dxdy) + c.C + s_PQ[center].x*s_PQ[center].x*s_PQ[center].y - (c.K + real_t(1.0)) * s_PQ[center].x;
            real_t dQdt = c.D_q * ( s_PQ[left].y + s_PQ[right].y + s_PQ[up].y + s_PQ[down].y - real_t(4.0)*s_PQ[center].y) / (c.dxdy) + s_PQ[center].x * c.K - s_PQ[center].x*s_PQ[center].x*s_PQ[center].y;
            c.d_PQdt[tid] = {dPdt, dQdt};
        }
        cg::sync(cg::this_thread_block());
    }
}
...
void integrate(Chemicals& c){
    for (int tid = (c.NX*c.NY) - ((threadIdx.x + blockIdx.x * blockDim.x) + (threadIdx.y + blockIdx.y * blockDim.y)*blockDim.x*gridDim.x) - 1 ; tid >= 0 ; tid -= gridDim.x*blockDim.x*gridDim.y*blockDim.y)
    { 
        c.d_PQ[tid] += c.dt * c.d_PQdt[tid];
    }
}
...
//Integrate forwards 1 timetstep
...
void simulate(const Sim_Configuration &config) {
    ...
    cudaLaunchCooperativeKernel((void*)kernel_simulate, grid, block, kernelargsremainder);
    cudaLaunchKernel((void*)prepare_buffer, grid, block,buffer_args);
