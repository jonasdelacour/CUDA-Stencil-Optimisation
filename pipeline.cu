...
void derivative(Chemicals& c, real2_t* s_PQ){
    ...
    {   
        ...
        __threadfence_block();
        if (valid_range){  
            real_t dPdt = c.D_p * ( s_PQ[left].x + s_PQ[right].x + s_PQ[up].x + s_PQ[down].x - real_t(4.0)*PQ_center.x) / (c.dxdy) + c.C + P2Q - (c.K + real_t(1.0)) * PQ_center.x;
            real_t dQdt = c.D_q * ( s_PQ[left].y + s_PQ[right].y + s_PQ[up].y + s_PQ[down].y - real_t(4.0)*PQ_center.y) / (c.dxdy) + PQ_center.x * c.K - P2Q;
