...
void derivative(Chemicals& c){
    real2_t* PQ = c.d_PQ;
    for (auto gtidy = threadIdx.y + blockIdx.y * blockDim.y + 1; gtidy < c.NY -1; gtidy += gridDim.y*blockDim.y)
    for (auto gtidx = threadIdx.x + blockIdx.x * blockDim.x + 1; gtidx < c.NX -1; gtidx += gridDim.x*blockDim.x)
    { 
        auto tid = gtidx + gtidy*c.NX;
        real_t dPdt = c.D_p * ( PQ[tid -1].x + PQ[tid +1].x + PQ[tid - c.NX].x + PQ[tid + c.NX].x - real_t(4.0)*PQ[tid].x) / (c.dx * c.dy) + c.C + PQ[tid].x*PQ[tid].x*PQ[tid].y - (c.K + real_t(1.0)) * PQ[tid].x;
        real_t dQdt = c.D_q * ( PQ[tid -1].y + PQ[tid +1].y + PQ[tid - c.NX].y + PQ[tid + c.NX].y - real_t(4.0)*PQ[tid].y) / (c.dx * c.dy) + PQ[tid].x * c.K - PQ[tid].x*PQ[tid].x*PQ[tid].y;
...
void simulate(const Sim_Configuration &config) {
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor((int*)&cu_nblocks,(void*)kernel_simulate,block.x*block.y,0);
    ...
    for (int i = 0; i < int(config.iter / config.data_period); i++){
        cudaLaunchCooperativeKernel((void*)kernel_simulate, grid, block, kernelargs,0, main_stream);
