...
void simulate(const Sim_Configuration &config) {
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor((int*)&cu_nblocks,(void*)integrate,block.x*block.y,0);
    ...
    for (int i = 0; i < int(config.iter / config.data_period); i++){
        ...
        cudaLaunchKernel((void*)prepare_buffer, grid, block,kernelargs);
